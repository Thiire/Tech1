...
char *restock(char *stock, char *result)
{
    int n = 0;
    int length = 0;
    char *restock;
    ...
    for (int n = 0; n < READ_SIZE + length; n++)
            restock[n] = '\0';
...
char *get_length(void)
{
    ...
    while (enter != 0) {
        if (stack[0] == '\0') {
            ...
            result = restock(stock, result);
        } else {
            result = restock(stack, result);
            modifie();
        }
        enter = to_confirm(result);
        if (enter == -1)
            return (NULL);
    }
    return (result);
